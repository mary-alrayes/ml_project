...
if __name__ == "__main__":
    ...
    param_grid = {
        ...
        'hidden_layers': [[x, y] for x in range(20, 30) for y in range(25, 36)],   # Number of neurons in the hidden layer
    ...
    plt.title("Training Loss and Accuracy Over Recorded Epochs, train/val")  # Plot title
    plt.legend()
    plt.grid(True)
...
#-------------------------------Assessment Set Predictions---------------------------------
    ...
    history = nn_assessment.fit(train_set_X, train_set_Y, X_val=assessment_X, y_val=assessment_Y, epochs=200, batch_size=32)
    ...
    plt.xlabel("Epochs")  # X-axis as the recorded epochs
    plt.ylabel("Value")  # Shared y-axis label
    plt.title("Training Loss and Accuracy Over Recorded Epochs, train/assess")  # Plot title
    plt.legend()
    plt.grid(True)
    ...
    ensemble_predictions = np.array(ensemble_predictions)  # Shape: (n_models, n_samples, n_targets)

    # Average the predictions across models
    ensemble_mean_predictions = np.mean(ensemble_predictions, axis=0)  # Shape: (n_samples, n_targets)
...
#-----------------------------Test set prediction---------------------------
    ...
    history = nn_final.fit(train_final_set_X, train_final_set_Y, X_val=assessment_X, y_val=assessment_Y, epochs=200, batch_size=32)
    #test preprocessing
    ...
    for top_model in top_models:
        ...
        # Retrain the model on train_set_X and train_set_Y
        retrained_final_model.fit(train_final_set_X, train_final_set_Y, epochs=200, batch_size=32)
    ...
    # Convert predictions to a NumPy array for easier manipulation
    ensemble_final_predictions = np.array(ensemble_final_predictions)  # Shape: (n_models, n_samples, n_targets)

    # Average the predictions across models
    ensemble_mean_final_predictions = np.mean(ensemble_final_predictions, axis=0)  # Shape: (n_samples, n_targets)
