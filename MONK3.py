...
if __name__ == "__main__":
    ...
    minority_class = resample(minority_class,
                              ...
                              random_state=63)  # For reproducibility

    # Combine the oversampled minority class with the majority class
    monk3_train_data = pd.concat([majority_class, minority_class])

    # Shuffle the balanced dataset
    monk3_train_data = monk3_train_data.sample(frac=1, random_state=62).reset_index(drop=True)
    ...
    best_params, best_score = search.grid_search(X, y, epoch=200, neurons=[4], output_size=1)
    print(f"Best Parameters:\n {best_params}, Best Score: {best_score}")

    # Define the network with dynamic hidden layers
    nn3 = CustomNeuralNetwork(input_size=X.shape[2],
                              hidden_layers=[4],
                              output_size=1,
    ...
    # Train the network
    history = nn3.fit(X, y, monk3_validation_X, monk3_validation_Y, epochs=200, batch_size=6)
    ...
    one_hot_test_monk3 = pd.concat([monk3_test_data['target'], encoded_columns_df], axis=1)

    # Verifica che tutte le colonne abbiano la stessa lunghezza
    assert all(encoded_columns_df[col].shape[0] == len(one_hot_test_monk3) for col in encoded_columns_df.columns), \
        "Le colonne codificate non hanno la stessa lunghezza!"

    monk3_real_test_X, monk3_real_test_Y = splitToFeaturesAndTargetClassification(one_hot_test_monk3)

    # Conversione a numpy array
    try:
        monk3_real_test_X = np.array(monk3_real_test_X, dtype=np.float64)  # Assicurati che siano numerici
